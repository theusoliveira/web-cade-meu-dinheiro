...
import { useCltDistribution } from "@/hooks/useCltDistribution";
...
export function CltDistributionClient() {
  ...
  const salaryInput = useCurrencyInput(salary, saveSalary);

  // ── Totals ─────────────────────────────────────────────────────────────────

  const totalAllocated = React.useMemo(
    () => categories.reduce((sum, cat) => sum + cat.items.reduce((s, i) => s + i.value, 0), 0),
    [categories],
  );

  const gastoLivre = salary - totalAllocated;

  function pct(value: number) {
    if (salary <= 0) return "—";
    return `${Math.round((value / salary) * 100)}%`;
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-lg font-semibold">Distribuição de Salário CLT</h1>
        <p className="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
          Planeje como distribuir o seu salário líquido.
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
        <h2 className="mb-4 text-sm font-semibold">Salário</h2>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-500">
              Salário após todas as alocações
            </p>
          </div>
        ...
        {salary > 0 && (
          <div className="mt-4 space-y-1">
            <div className="flex justify-between text-[11px] text-zinc-500">
              ...
              <span>{pct(totalAllocated)} do salário</span>
            </div>
